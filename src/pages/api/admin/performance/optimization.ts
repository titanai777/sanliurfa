  ...
  try {
    return await withAdminOpsReadAccess({
      ...
      onDenied: (_response, statusCode, duration) => {
        recordRequest('GET', '/api/admin/performance/optimization', statusCode, duration);
      },
      ...
      logger.info('Performance optimization data retrieved', { slowQueries: slowQueries.length, duration: Date.now() - startTime });
